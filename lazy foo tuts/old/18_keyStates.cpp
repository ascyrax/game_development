...
void runGameLoop()
{
    ...
    while (!quitGameLoop)
    {
        ...
        const uint8_t *arr = SDL_GetKeyboardState(NULL);

        // clear screen
        if (arr[SDL_SCANCODE_UP])
            SDL_SetRenderDrawColor(gRenderer, 0x0, 0x0, 0x0, 1);
        else if (arr[SDL_SCANCODE_DOWN])
            SDL_SetRenderDrawColor(gRenderer, 0xff, 0x0, 0x0, 1);
        else if (arr[SDL_SCANCODE_RIGHT])
            SDL_SetRenderDrawColor(gRenderer, 0x0, 0xff, 0x0, 1);
        else if (arr[SDL_SCANCODE_LEFT])
            SDL_SetRenderDrawColor(gRenderer, 0x0, 0x0, 0xff, 1);
        else
